...
export const updateNews = async (id, newsData) => {
  try {
    // For FormData with files, we need to use POST with _method: 'PUT'
    if (newsData instanceof FormData) {
      newsData.append("_method", "PUT");
      const response = await axios.post(`/api/news/${id}`, newsData);
      return response.data;
    } else {
      // For regular JSON data, we can use PUT directly
