...
const StudentRegistrationForm = () => {
  ...
  const gradeOptions = [
    { value: "TC-S", label: t("registration.academicLevels.tcSciences") },
    { value: "TC-LSH", label: t("registration.academicLevels.tcHumanities") },
    { value: "1BAC-SE", label: t("registration.academicLevels.1bacSciExp") },
    { value: "1BAC-LSH", label: t("registration.academicLevels.1bacHumanities") },
    { value: "2BAC-PC", label: t("registration.academicLevels.2bacPhysicsChem") },
    { value: "2BAC-SVT", label: t("registration.academicLevels.2bacLifeSciences") },
    { value: "2BAC-SH", label: t("registration.academicLevels.2bacHumanities") },
    { value: "2BAC-L", label: t("registration.academicLevels.2bacLiterature") },
  ];
  ...
  const validateForm = () => {
    ...
    if (
      ...
      !/^\d{8,15}$/.test(formData.studentPhone.replace(/\D/g, ""))
    ) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const dataToSubmit = {
        ...
        deathDate:
          formData.civilStatus === "orphan" ? formData.deathDate : null,
      ...
      setError(err.response?.data?.message || t("errors.submissionError"));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const downloadPDF = async () => {
    try {
      ...
      link.setAttribute(
        "download",
        `inscription_${formData.studentName.replace(/\s+/g, "_")}.pdf`
      );
        ...
        {!isSubmitted ? (
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                >
                  <option value="">{t("registration.academicLevel")}</option>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder={t("registration.placeholders.phoneExample")}
                />
                {validationErrors.studentPhone && (
                  <p className="text-red-500 text-xs mt-1">
                    {validationErrors.studentPhone}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  {t("registration.parentProfession")}:{" "}
                  <span className="text-red-500">*</span>
                </label>
                <input
                  type="text"
            ...
            <h2 className="text-lg font-semibold text-gray-700 border-b pb-2 mt-8">
              {t("registration.addressAndFamily")}
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full p-2 border border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500"
                >
                  <option value="together">{t("registration.civilStatusType.together")}</option>
                  <option value="divorced">{t("registration.civilStatusType.divorced")}</option>
                  <option value="orphan">{t("registration.civilStatusType.orphan")}</option>
                </select>
              </div>
            ...
            <div className="mt-6">
              ...
              <textarea
                ...
                className="w-full p-2 border border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500"
                rows="3"
                placeholder={t("registration.placeholders.additionalNotes")}
              ></textarea>
            </div>
            ...
            <div className="flex justify-end gap-4">
              ...
              <button
                ...
                disabled={isLoading}
              >
                {isLoading ? t("registration.buttons.processing") : t("registration.buttons.submit")}
              </button>
            </div>
          </form>
        ) : (
          <div className="text-center">
            <div className="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded mb-6">
              <p className="font-bold">{t("registration.success.registrationSuccess")}</p>
              <p>{t("registration.success.formSubmitted")}</p>
            </div>

            <div className="mb-6">
              <h3 className="text-md font-medium mb-2">
                {t("registration.download.options")}:
              </h3>
              <div className="flex justify-center gap-4 mb-4">
                <label className="inline-flex items-center">
                  ...
                  />
                  <span className="ml-2">{t("registration.download.standardVersion")}</span>
                </label>
                {/* <label className="inline-flex items-center">
                  <input 
                    type="radio" 
            ...
            <div className="flex justify-center gap-4">
              <button
                ...
                disabled={isLoading}
              >
                {isLoading ? t("registration.buttons.generating") : t("registration.buttons.downloadPDF")}
                <svg
              ...
              >
                {t("registration.buttons.backToHome")}
              </button>
            </div>
          </div>
        )}
